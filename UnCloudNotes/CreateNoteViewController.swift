...
class CreateNoteViewController: UIViewController, UsesCoreDataObjects {
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    guard let image = note?.image else { // if image was added to the note object, we display it
      titleField.becomeFirstResponder()
      return
    }
    attachedPhoto.image = image //tell the attachedPhoto IBOutlet to set its image property to image
